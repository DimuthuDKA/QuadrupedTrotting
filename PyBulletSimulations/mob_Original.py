...
class Simulation(threading.Thread):
	def __init__(self, sim, gui=False):
	...
	def setFollowCamera(self, newval, zoom=1):
	...
	def run(self):
		...
		p.setGravity(0,0,-15)
		planeId = p.loadURDF("plane100.urdf")    #planeMod.urdf
		...
		if self.simtype == "walker":
            ...
            #self.ur = p.loadURDF("C:/Users/Dimuthu/OneDrive - DePaul University/Depaul University/Research/4-Legged Robot/Trotting/PybulletWork/jgrey/walkerTim.urdf",spineStartPos, spineStartOrientation, flags=p.URDF_USE_SELF_COLLISION)
			self.ur = p.loadURDF("walkerTim.urdf",spineStartPos, spineStartOrientation, flags=p.URDF_USE_SELF_COLLISION)
		elif self.simtype == "snake":
			spineStartPos = [0,0,0.15]
			spineStartOrientation = p.getQuaternionFromEuler([math.pi / 2,0,0])

			self.ur = p.loadURDF("C:/python/snake2.urdf",spineStartPos, spineStartOrientation, flags=p.URDF_USE_SELF_COLLISION)
		elif self.simtype == "snake2":
			spineStartPos = [0,0,0.15]
			spineStartOrientation = p.getQuaternionFromEuler([math.pi / 2,0,0])
			self.ur = p.loadURDF("C:/python/snake3.urdf",spineStartPos, spineStartOrientation, flags=p.URDF_USE_SELF_COLLISION)
		self.valid = True
		...
		while i < 2400:
			...
			if self.followcam and self.valid :
				p.resetDebugVisualizerCamera(cameraDistance = self.zoom, cameraYaw = 180, cameraPitch = -45, cameraTargetPosition =p.getLinkState(self.ur, 0)[0])
               
			# Top Down View
			# if self.followcam and self.valid :
				# p.resetDebugVisualizerCamera(cameraDistance = self.zoom, cameraYaw = 90, cameraPitch = -80, cameraTargetPosition = p.getLinkState(self.ur, 0)[0])

			# Side view
			#if self.followcam and self.valid :
				#p.resetDebugVisualizerCamera(cameraDistance = self.zoom, cameraYaw = 90, cameraPitch = 0, cameraTargetPosition = p.getLinkState(self.ur, 0)[0])
